<template>
	<div class="rank">
		...
		<div v-show="!rankInfo.length" class="load">
			<van-loading size="24px" color="#1989fa">加载中...</van-loading>
		</div>
		<div class="rankContent" v-show="rankInfo.length">
			<h4 class="title">官方榜</h4>
			<div @click="rankClick(item.id)" class="officialBox" v-for="(item,index) in rankInfo.slice(0,4)" :key="index">
				<div class="rankPic">
					<van-image width="6rem" height="6rem" fit="cover" radius="5" :src="item.coverImgUrl" />
					<!-- <p class="updateTime">{{item.updateTime}}</p> -->
				</div>
				<div class="rankInfo">
					<p class="van-ellipsis" v-for="(value,indey) in item.tracks" :key="value.id">{{indey+1}}.{{value.first}}—{{value.second}}</p>
				</div>
			</div>
